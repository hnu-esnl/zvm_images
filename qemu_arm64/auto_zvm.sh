...
if [ "$OPS" = "${ops_array[0]}" ]; then
    if [ "$PLAT" = "${plat_array[0]}" ]; then
        strace -o qemu_bug.txt ./qemu-system-aarch64 \
        ...
        -device loader,file=./zephyr/zephyr.bin,addr=0x60000000,force-raw=on \
        -device loader,file=./linux/Image_withFS,addr=0x70000000,force-raw=on \
        -device loader,file=./linux/linux-qemu-virt.dtb,addr=0x75000000 \
        -kernel ./zvm_host.elf
